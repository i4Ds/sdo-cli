...
from trixi.logger import PytorchExperimentLogger, PytorchVisdomLogger, TelegramMessageLogger, PytorchPlotFileLogger
...
class PytorchExperimentStub:
    def __init__(self, base_dir=None, name=None, config=None, loggers=None):
        ...
        if base_dir is not None:
            self.elog = PytorchExperimentLogger(
                base_dir=self.base_dir, exp_name=self.name)

            self.results = ResultLogDict(
                "results-log.json", base_dir=self.elog.result_dir)
        else:
        ...
        if log_type == "tensorboard":
            if "target_dir" not in log_params or log_params["target_dir"] is None:
                if self.elog is not None and not isinstance(self.elog, Mock):
                    log_params["target_dir"] = os.path.join(
                        self.elog.save_dir, "tensorboard")
                else:
                    raise AttributeError(
                        "TensorboardLogger requires a target_dir or an ExperimentLogger instance.")
            elif self.elog is not None and not isinstance(self.elog, Mock):
                log_params["target_dir"] = os.path.join(
                    log_params["target_dir"], self.elog.folder_name)

        if log_type == "file":
            if "img_dir" not in log_params or log_params["img_dir"] is None:
                log_params["img_dir"] = os.path.join(
                    self.elog.save_dir, "imgs")
                if "plot_dir" not in log_params or log_params["plot_dir"] is None:
                    log_params["plot_dir"] = os.path.join(
                        self.elog.save_dir, "plots")
    ...
    def _save_exp_config(self):
        ...
        if self.elog is not None and not isinstance(self.elog, Mock):
            ...
            self.elog.save_config(
                Config(**{"name": self.name, "time": cur_time,
                          "state": "Stub", "current_time": cur_time, "epoch": 0}),
                "exp",
            )
    ...
    def add_result(self, value, name, counter=None, tag=None, label=None, plot_result=True, plot_running_mean=False):
        ...
        **This should be your preferred method to log your numeric values**
        ...
        r_elem = ResultElement(data=value, label=tag_name,
                               epoch=0, counter=counter)
        ...
        if plot_result:
            ...
            self.elog.show_value(
                value=value, name=name, tag=tag_name, counter=counter, show_legend=legend)
